...
class DaoImagenAnexo {
    
    var sql: SQLImagenAnexo;
    var conexion: Conexion;
    
    init(conexion: Conexion) {
        self.conexion = conexion;
        self.sql = SQLImagenAnexo(db: self.conexion.obtenerConexion());
    }
    ...
    func crearTabla() {
        do {
            ...
            print("Error crear ImagenAnexo: \(error)")
        }
    }
    
    /**
     Permite insertar un conjunto de datos.
     - Parameter data: Corresponde a una lista de objetos ImagenAnexo.
     - Returns: -1 si el registro fue incompleto, : 0 si no se registró ninún dato, : 1 si el registro fue exitoso
     */
    func insertarRegistros(data: [ImagenAnexo]) -> Int {
        do {
            let result = try self.sql.insertarRegistros(data: data);
            return result;
        } catch {
            print("Error insertar registros ImagenAnexo: \(error)")
            return 0;
        }
    }
     ...
     */
    func actualizarRegistro(idRegistro: Int, data: ImagenAnexo) -> Int {
        do {
            let result = try self.sql.actualizarRegistro(idRegistro: idRegistro, data: data);
            return result;
        } catch {
            print("Error actualizar ImagenAnexo: \(error)");
            return 0;
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve una lista de objetos ImagenAnexo.
     */
    func seleccionarRegistrosTodo() -> [ImagenAnexo] {
        do {
            let result = try self.sql.seleccionarRegistrosTodo();
            return result;
        } catch {
            print("Error seleccionar todo ImagenAnexo: \(error)");
            return [ImagenAnexo]();
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve un objeto ImagenAnexo opcional.
     */
    func seleccionarRegistroPorId(idRegistro: Int) -> ImagenAnexo? {
        do {
            let result = try self.sql.seleccionarRegistrosPorId(idRegistro: idRegistro);
            return result;
        } catch {
            print("Error seleccionar por id ImagenAnexo: \(error)")
            return nil;
        }
    }
    ...
    func eliminarRegistro(idRegistro: Int) -> Int {
        do {
            ...
            print("Error eliminar ImagenAnexo: \(error)");
            return 0;
        }
    }
