...
class DaoConstante {
    ...
    var sql: SQLConstante;
    var conexion: Conexion;
    
    init(conexion: Conexion) {
        self.conexion = conexion;
        self.sql = SQLConstante(db: self.conexion.obtenerConexion());
    }
    ...
    func crearTabla() {
        do {
            ...
            print("Error crear Constante: \(error)")
        }
    }
     ...
     */
    func insertarRegistros(data: [Constante]) -> Int {
        do {
            let result = try self.sql.insertarRegistros(data: data);
            return result;
        } catch {
            print("Error insertar registros Constante: \(error)")
            return 0;
        }
    }
     ...
     */
    func actualizarRegistro(idRegistro: Int, data: Constante) -> Int {
        do {
            let result = try self.sql.actualizarRegistro(idRegistro: idRegistro, data: data);
            return result;
        } catch {
            print("Error actualizar Constante: \(error)");
            return 0;
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve una lista de objetos Constante.
     */
    func seleccionarRegistrosTodo() -> [Constante] {
        do {
            let result = try self.sql.seleccionarRegistrosTodo();
            return result;
        } catch {
            print("Error seleccionar todo Constante: \(error)");
            return [Constante]();
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve un objeto Constante opcional.
     */
    func seleccionarRegistroPorId(idRegistro: Int) -> Constante? {
        do {
            let result = try self.sql.seleccionarRegistrosPorId(idRegistro: idRegistro);
            return result;
        } catch {
            print("Error seleccionar por id Constante: \(error)")
            return nil;
        }
    }
    ...
    func eliminarRegistro(idRegistro: Int) -> Int {
        do {
            ...
            print("Error eliminar Constante: \(error)");
            return 0;
        }
    }
