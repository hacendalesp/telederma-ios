...
class DaoHelpDesk {
    ...
    init(conexion: Conexion) {
        ...
        self.sql = SQLHelpDesk(db: self.conexion.obtenerConexion());
    }
    ...
    func crearTabla() {
        do {
            ...
            print("Error crear HelpDesk: \(error)")
        }
    }
     ...
     */
    func insertarRegistros(data: [HelpDesk]) -> Int {
        do {
            let result = try self.sql.insertarRegistros(data: data);
            return result;
        } catch {
            print("Error insertar registros HelpDesk: \(error)")
            return 0;
        }
    }
     ...
     */
    func actualizarRegistro(idRegistro: Int, data: HelpDesk) -> Int {
        do {
            let result = try self.sql.actualizarRegistro(idRegistro: idRegistro, data: data);
            return result;
        } catch {
            print("Error actualizar HelpDesk: \(error)");
            return 0;
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve una lista de objetos HelpDesk.
     */
    func seleccionarRegistrosTodo() -> [HelpDesk] {
        do {
            let result = try self.sql.seleccionarRegistrosTodo();
            return result;
        } catch {
            print("Error seleccionar todo HelpDesk: \(error)");
            return [HelpDesk]();
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve un objeto HelpDesk opcional.
     */
    func seleccionarRegistroPorId(idRegistro: Int) -> HelpDesk? {
        do {
            let result = try self.sql.seleccionarRegistrosPorId(idRegistro: idRegistro);
            return result;
        } catch {
            print("Error seleccionar por id HelpDesk: \(error)")
            return nil;
        }
    }
    ...
    func eliminarRegistro(idRegistro: Int) -> Int {
        do {
            ...
            print("Error eliminar HelpDesk: \(error)");
            return 0;
        }
    }
