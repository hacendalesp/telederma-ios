...
class DaoExamenSolicitado {
    
    var sql: SQLExamenSolicitado;
    var conexion: Conexion;
    
    init(conexion: Conexion) {
        self.conexion = conexion;
        self.sql = SQLExamenSolicitado(db: self.conexion.obtenerConexion());
    }
    ...
    func crearTabla() {
        do {
            ...
            print("Error crear ExamenSolicitado: \(error)")
        }
    }
     ...
     */
    func insertarRegistros(data: [ExamenSolicitado]) -> Int {
        do {
            let result = try self.sql.insertarRegistros(data: data);
            return result;
        } catch {
            print("Error insertar registros ExamenSolicitado: \(error)")
            return 0;
        }
    }
     ...
     */
    func actualizarRegistro(idRegistro: Int, data: ExamenSolicitado) -> Int {
        do {
            let result = try self.sql.actualizarRegistro(idRegistro: idRegistro, data: data);
            return result;
        } catch {
            print("Error actualizar ExamenSolicitado: \(error)");
            return 0;
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve una lista de objetos ExamenSolicitado.
     */
    func seleccionarRegistrosTodo() -> [ExamenSolicitado] {
        do {
            let result = try self.sql.seleccionarRegistrosTodo();
            return result;
        } catch {
            print("Error seleccionar todo ExamenSolicitado: \(error)");
    ...
    /**
     ...
     - Returns: Devuelve un objeto ExamenSolicitado opcional.
     */
    func seleccionarRegistroPorId(idRegistro: Int) -> ExamenSolicitado? {
        do {
            let result = try self.sql.seleccionarRegistrosPorId(idRegistro: idRegistro);
            return result;
        } catch {
            print("Error seleccionar por id ExamenSolicitado: \(error)")
            return nil;
        }
    }
    ...
    func eliminarRegistro(idRegistro: Int) -> Int {
        do {
            ...
            print("Error eliminar ExamenSolicitado: \(error)");
            return 0;
        }
    }
