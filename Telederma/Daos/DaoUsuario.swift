...
class DaoUsuario {
    ...
    init(conexion: Conexion) {
        ...
        self.sql = SQLUsuario(db: self.conexion.obtenerConexion());
    }
    ...
    func crearTabla() {
        do {
            ...
            print("Error crear Usuario: \(error)")
        }
    }
     ...
     */
    func insertarRegistros(data: [Usuario]) -> Int {
        do {
            let result = try self.sql.insertarRegistros(data: data);
            return result;
        } catch {
            print("Error insertar registros Usuario: \(error)")
            return 0;
        }
    }
     ...
     */
    func actualizarRegistro(idRegistro: Int, data: Usuario) -> Int {
        do {
            let result = try self.sql.actualizarRegistro(idRegistro: idRegistro, data: data);
            return result;
        } catch {
            print("Error actualizar Usuario: \(error)");
            return 0;
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve una lista de objetos Usuario.
     */
    func seleccionarRegistrosTodo() -> [Usuario] {
        do {
            let result = try self.sql.seleccionarRegistrosTodo();
            return result;
        } catch {
            print("Error seleccionar todo Usuario: \(error)");
            return [Usuario]();
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve un objeto Usuario opcional.
     */
    func seleccionarRegistroPorId(idRegistro: Int) -> Usuario? {
        do {
            let result = try self.sql.seleccionarRegistrosPorId(idRegistro: idRegistro);
            return result;
        } catch {
            print("Error seleccionar por id Usuario: \(error)")
            return nil;
        }
    }
    ...
    func eliminarRegistro(idRegistro: Int) -> Int {
        do {
            ...
            print("Error eliminar Usuario: \(error)");
            return 0;
        }
    }
