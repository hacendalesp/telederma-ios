...
class DaoMipres {
    ...
    init(conexion: Conexion) {
        ...
        self.sql = SQLMipres(db: self.conexion.obtenerConexion());
    }
    ...
    func crearTabla() {
        do {
            ...
            print("Error crear Mipres: \(error)")
        }
    }
     ...
     */
    func insertarRegistros(data: [Mipres]) -> Int {
        do {
            let result = try self.sql.insertarRegistros(data: data);
            return result;
        } catch {
            print("Error insertar registros Mipres: \(error)")
            return 0;
        }
    }
     ...
     */
    func actualizarRegistro(idRegistro: Int, data: Mipres) -> Int {
        do {
            let result = try self.sql.actualizarRegistro(idRegistro: idRegistro, data: data);
            return result;
        } catch {
            print("Error actualizar Mipres: \(error)");
            return 0;
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve una lista de objetos Mipres.
     */
    func seleccionarRegistrosTodo() -> [Mipres] {
        do {
            let result = try self.sql.seleccionarRegistrosTodo();
            return result;
        } catch {
            print("Error seleccionar todo Mipres: \(error)");
            return [Mipres]();
        }
    }
    ...
    /**
     ...
     - Returns: Devuelve un objeto Mipres opcional.
     */
    func seleccionarRegistroPorId(idRegistro: Int) -> Mipres? {
        do {
            let result = try self.sql.seleccionarRegistrosPorId(idRegistro: idRegistro);
            return result;
        } catch {
            print("Error seleccionar por id Mipres: \(error)")
            return nil;
        }
    }
    ...
    func eliminarRegistro(idRegistro: Int) -> Int {
        do {
            ...
            print("Error eliminar Mipres: \(error)");
            return 0;
        }
    }
