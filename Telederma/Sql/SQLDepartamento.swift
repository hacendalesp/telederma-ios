...
class SQLDepartamento: SQLBase {
    ...
    override init(db: Connection?) {
        ...
        super.init(db: db);
        self.tabla = Table("departamentos");
    }
     ...
     */
    func insertarRegistros (data: [Departamento]) throws -> Int {
        if let db = self.database {
            if (data.count > 0) {
                var contador = 0;
                for item in data {
     ...
     */
    func actualizarRegistro (idRegistro: Int, data: Departamento) throws -> Int{
        if let db = self.database {
    ...
    /**
     ...
     - Returns: Retorna un objeto Departamento
     */
    func seleccionarRegistrosPorId (idRegistro: Int) throws -> Departamento? {
        var obj: Departamento? = nil;
        if let db = self.database {
