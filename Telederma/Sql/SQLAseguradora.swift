...
class SQLAseguradora: SQLBase {
     ...
     */
    func insertarRegistros (data: [Aseguradora]) throws -> Int {
        if let db = self.database {
            if (data.count > 0) {
                var contador = 0;
                for item in data {
     ...
     */
    func actualizarRegistro (idRegistro: Int, data: Aseguradora) throws -> Int{
        if let db = self.database {
     ...
     */
    func seleccionarRegistrosTodo () throws -> [Aseguradora] {
        var datos = [Aseguradora]();
        if let db = self.database {
    ...
    /**
     ...
     - Returns: Retorna un objeto Aseguradora
     */
    func seleccionarRegistrosPorId (idRegistro: Int) throws -> Aseguradora? {
        var obj: Aseguradora? = nil;
        if let db = self.database {
    ...
    /**
     Método que permite obtener toda la información de un conjunto de registros específico por coincidencias del name.
     - Parameter name: Corresponde al valor del campo name que se desea obtener las coincidencias.
     - Returns: Retorna una lista de objetos Aseguradora
     */
    func seleccionarRegistrosPorName (cadena: String) throws -> [Aseguradora] {
        var aseguradoras = [Aseguradora]();
        if (cadena != ""){
            if let db = self.database {
                let query = self.tabla!.select(*).filter(name.like(Expression.init("%\(cadena)%"))).order(name.asc).limit(5);
    ...
    /**
     ...
     - Returns: Retorna un objeto Aseguradora
     */
    func seleccionarUnRegistroPorNombre (nombre: String) throws -> Aseguradora? {
        var obj: Aseguradora? = nil;
        if let db = self.database {
