...
//  Telederma
//
//  Created by Pedro Erazo Acosta on 27/03/20.
...
class SQLMipres: SQLBase {
     ...
     */
    func insertarRegistros (data: [Mipres]) throws -> Int {
        if let db = self.database {
            if (data.count > 0) {
                var contador = 0;
                for item in data {
     ...
     */
    func actualizarRegistro (idRegistro: Int, data: Mipres) throws -> Int{
        if let db = self.database {
    ...
    /**
     ...
     - Returns: Retorna un objeto Mipres
     */
    func seleccionarRegistrosPorId (idRegistro: Int) throws -> Mipres? {
        var obj: Mipres? = nil;
        if let db = self.database {
